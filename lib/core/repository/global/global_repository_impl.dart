...
import '../../constant/shared_preferences_consts.dart';
import '../../network/exceptions/shared_preference_exception.dart';
...
class GlobalRepositoryImpl implements GlobalRepository {
  ...
  Future<Locale?> appLang() async {
    try {
      ...
      if (lang != null) {
        result = lang == 'en' ? const Locale('en', 'US') : const Locale('ar', 'EG');
      }
      return result;
    } catch (e) {
      debugPrint('$e');
  ...
  Future<String?> getFontSize() async {
    try {
      String? fontSize = await helper.getString(key: Current_Font_Size_Key);
      return fontSize;
    } catch (e) {
      debugPrint('$e');
      return null;
